...
\caption{\label{fig:1eNp:box:RUN1:intro}Box-cuts for \npsel channel.}
\end{center}
\end{figure}

\par The analysis presented in this note is at a very mature state. A particular strength consists in the multiple sideband channels available to validate the simulation, reconstruction, and help constrain $\nu_e$ interaction uncertainties. A high statistics $\pi^0$ sideband (section~\ref{sec:controls:pi0}) provides confidence in the performance of the analysis' shower reconstruction. A high-quality inclusive contained $\nu_{\mu}$ selection shows good data-simulation agreement in both Run 1 and Run 3 and provides the input for the $\nu_{\mu}$ constraint which has the primary goal of reducing systematic uncertainties for low-energy $\nu_e$s. A primary estimation of the impact of the $\nu_{\mu}$ constraint shows a 50\% reduction in modeling uncertainties, though this is to be revisited by the time of the February collaboration meeting. Additionally, an inclusive $\nu_{\mu}$ selection (section~\ref{sec:nueselection:inclusive}) provides a valuable starting point for many possible final-state measurements which may become necessary to further validate the analysis as its review progresses. Finally, on the $\nu_e$ side, the three selections developed all provide valuable input to the analysis. The $\nu_e$ inclusive channel has demonstrated the ability to select a high-statistics sample of $\mathcal{O}$(300 events) which can provide a robust measurement of the intrinsic $\nu_e$ content of the BNB beam, particularly at higher energies. This measurement is essential in order to validate a potential claim of new physics at lower energy. Finally, the 1$e$0$p$0$\pi$ channel has matured significantly, leading to a 50\% pure selection which will be able to further constrain reconstruction and modeling uncertainties associated to proton reconstruction and N$p \rightarrow 0p$ event migration.
