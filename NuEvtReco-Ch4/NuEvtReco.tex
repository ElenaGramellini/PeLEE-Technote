...
However, as long as the same procedure with the same likelihood is applied to both the data and the simulation, no systematic uncertainty due to this mis-modelling is expected.
Systematic uncertainties arise from the fact that the measured distribution of $dE/dx$ in the data differs from the simulation, or because the properties of the tracks we are considering are non properly simulated, such as angular or length distributions.
This behaviour is qualitatively present in all the PID methods.
Regardless of systematic uncertainties, in a sample of contained protons and muons used to test the performance, this PID variable is able to reach 94\% muon efficiency, with 10\% proton mis-ID rate.
