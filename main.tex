...
% \end{enumerate}
\textcolor{blue}{The statistical-only sensitivity is measured using the result of the box-cut 1$e$N$p$ selection (figure~\ref{fig:1eNp:box:RUN1}). The distribution of the test-statistic computed with SBNFit ($\Delta\chi^2$, eq.~\ref{eqn:deltachi2}) and the stand-alone tool  Figure~\ref{fig:1eNp:box:statonlysensitivity}.  The left panel shows the sensitivity estimated via SBNFit with the }
