...
\begin{figure}[H]
    \begin{center}
     \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=1.00\textwidth]{Sensitivity/numuconstraint-truth/SBNfit_Cls_nue_nu_e_truth_1eNp_LEE_deltachi_statonly.pdf}
    \label{fig:nue_statonly}
    \caption{statistical only sensitivity estimate for 1eNp truth level selection (without constraint)}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=1.00\textwidth]{Sensitivity/numuconstraint-truth/SBNfit_Cls_nue_nu_e_truth_1eNp_LEE_syst.pdf}
    \label{fig:nue_statsyst}
    \caption{statistical and systematics sensitivity estimate for 1eNp truth level selection (without constraint)}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
    \includegraphics[width=1.00\textwidth]{Sensitivity/numuconstraint-truth/SBNfit_Cls_nuenumu_nu_e_truth_1eNp_LEE_deltachi_statonly.pdf}
    \label{fig:nuenumu_statonly}
    \caption{statistical only sensitivity estimate for 1eNp truth level selection}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=1.00\textwidth]{Sensitivity/numuconstraint-truth/SBNfit_Cls_nuenumu_nu_e_truth_1eNp_LEE_syst.pdf}
    \label{fig:nuenumu_statsyst}
    \caption{statistical and systematics sensitivity estimate for 1eNp truth level selection with the $\nu_{\mu} constraint$}
    \end{subfigure}
    \caption{The sensitivity estimate for the 1eNp truth level selection for statistical only estimate and the statistical+systematics estimate.}.
    \label{fig:sensitivity_numuconst}
    \caption{Truth-level sensitivity estimate comparisons for the 1eNp selection before constraint is included. It has been observed that there is less constraint power on the sensitivity estimate when we include the MC statistical errors ($\sqrt{N_{bin}}$) as opposed to what is shown on Fig.~\ref{fig:numuconstraintresult}. We have also studied the kinematic topology for the selection and observed that when we select more hadrons in the final state of interaction, there is less correlation that we can exploit between the two channels. This leads to similar sensitivity estimates before and after applying the $\nu_\mu$ constraint.}
    \end{center}
\end{figure}
...
\newpage
\section{Outlook: Remaining Steps in the Analysis}
