...
\abstract{The analysis presented in this note uses data from the MicroBooNE Experiment to measure  $\nu_e$ interactions from the Booster Neutrino Beamline (BNB) using the Pandora reconstruction framework. Our goal is to address the nature of the anomalous excess of low-energy events (LEE) with electro-magnetic activity in the final state as recorded by the MiniBooNE experiment. In this document, we outline the analysis strategy, focusing on the reconstruction and event selections performed, the evaluation of systematics associated to detector, flux, and neutrino-interaction modeling, and the expected sensitivity to the MiniBooNE-$\nu_e$ LEE model using the available BNB dataset of 10.1$\times 10^{20}$ POT. }
...
\subsection{Input to \npsel and \zpsel and channel orthogonality }
...
\begin{center}
    ...
    \end{subfigure}
\caption{\label{ffig:1eNp:bdt:var} BDT variables importance in terms of ``total gain". In \texttt{xgboost}~\cite{bib:xgboost}, "gain" is the improvement in accuracy brought by a feature to the branches it is on; "total gain" refers to the sum of the "gain" across all branches. dEdx variables are combined for different planes (note the ``ALL" suffix). Left: total gain value. Right: ranking based on the total gain value (highest ranking=15, lowest=1).}
\end{center}
\end{figure}
...
The \zpsel channel can also be used to constrain uncertainties associated with the \npsel selection.  In particular, it can be used to constrain uncertainties associated with low energy protons, such as the modeling of the proton multiplicity and kinematics produced by neutrino interactions, as well as their reconstruction efficiencies.  This selection is very sensitive to the neutrino interaction modeling, which makes it a useful way to constrain these systematics with data. As an explicit example for why targeting such a channel is important is the fact that in MCC8, there were twice as many events in this channel as in MCC9, including half of the  electron neutrinos below 0.8 GeV.
...
The data-MC agreement on run 3 data of the BDT response after a slice and after the \zpsel pre-selection is shown in Fig.~\ref{fig:1e0p:bdt:bkgscore} The agreement is generally good.  

The BDT response after the loose selection is shown in Fig.~\ref{fig:1e0p:bdt:loose}.  Based on this plot events above 0.96 in BDT response are selected as signal.  The final BDT selection is the loose cuts defined in Table~\ref{tab:1e0p:loosecut} with the addition of this cut on the BDT response. 
...
The flux model and its uncertainties for MCC9~\cite{bib:fluxmcc9,bib:fluxtechnote} are largely the same as MCC8~\cite{bib:fluxtechnote}. We use the multisim technique to estimate the flux uncertainties. The various sources of uncertainties are taken as independent and are varied individually around the central value. They are listed in Table~\ref{tab:fluxsyst} along the number of universes considered for each. The fractional covariance matrix and correlation matrix for the 1eNp selection are presented in Figure~\ref{fig:fluxmatrices} and show smaller uncertainties than expected, ~7\% uncertainty in the lowest visible energy bin (200-300 MeV) and 2-4\% in other bins. \textit{The evaluation of how flux-uncertainties are incorporated in the correlation matrix is therefore under further investigation}. Meanwhile, Figure~\ref{fig:fluxsystvars} displays the effects of reweighting central value in each universe of individual example parameters of the flux model.
...
\begin{center}
    \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=1.00\textwidth]{CovarianceMatrices/SBNfit_fractional_covariance_matrix_nue_reco_e_scaledtofullpot_mc_collapsed.pdf}
    \caption{fractional covariance matrix}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=1.00\textwidth]{CovarianceMatrices/SBNfit_correlation_matrix_nue_reco_e_scaledtofullpot_mc_collapsed.pdf}
...
\begin{center}
    ...
    \begin{subfigure}[b]{0.33\textwidth}
    \centering
    \includegraphics[width=1.00\textwidth]{systvariations/Variation_nue_reco_e_genietune_run1_fluxonly_nucleoninexsec_FluxUnisim_nu_uBooNE_nue_intrinsic.pdf}
    %\caption{\label{fig:numuconstraint:xsec}different xsec models for CC interactions.}
    \end{subfigure}
    \begin{subfigure}[b]{0.33\textwidth}
    \centering
...
% \end{enumerate}
The statistical-only sensitivity is measured using the result of the box-cut 1$e$N$p$ selection (figure~\ref{fig:1eNp:box:RUN1}). The distribution of the test-statistic computed with SBNFit ($\Delta\chi^2$, eq.~\ref{eqn:deltachi2}) and the stand-alone tool \texttt{LLR Poisson} (eq.~\ref{eqn:llrpois}) are shown on the left and right-panel of figure~\ref{fig:1eNp:box:statonlysensitivity}, respectively. The stand-alone tool was also used to validate that, when using the $\Delta\chi^2$ test-statistic, results from SBNFit's statistics-only sensitivity calculation can be fully reproduced.
...
\par This preliminary exploration of the analysis' sensitivity to 3+1 oscillation models will be further explored as the analysis progresses in the coming weeks.
...
This section includes the data-MC comparisons and efficiencies plot for the $\nu_e$ CC inclusive selection which where omitted in \cref{sec:nueselection:inclusive}.
\begin{figure}[H]
    \centering
    \includegraphics[height=4.5cm]{NueCCsel/Images/run1/pre_vtx.pdf}
    \includegraphics[height=4.5cm]{NueCCsel/Images/run1/pre_angles.pdf}
    \caption{Vertex position of neutrino candidate and angular distributions of electron candidate before selection.}
    \label{fig:nuecc:pre_sel}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[height=5cm]{NueCCsel/Images/run1/efficiency_cat_2.pdf}
    \includegraphics[height=5cm]{NueCCsel/Images/run1/efficiency_int.pdf}
    \caption{Efficiencies at any stage in the selection, broken down in interaction types and final states.}
    \label{fig:nuecc:eff_all}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[height=5cm]{NueCCsel/Images/run1/e_cand_dedx.pdf}
    \includegraphics[height=5cm]{NueCCsel/Images/run1/e_cand_secondshower.pdf}
    \includegraphics[height=5cm]{NueCCsel/Images/run1/e_cand_subclusters.pdf}
    \includegraphics[height=5cm]{NueCCsel/Images/run1/e_cand_dist.pdf}
    \caption{Variables used for electron identification.}
    \label{fig:nuecc:e_cand}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[height=5cm]{NueCCsel/Images/run1/pre_daughter_1.pdf}
    \includegraphics[height=5cm]{NueCCsel/Images/run1/pre_daughter_2.pdf}
    \caption{Variables used for classify the other daughters.}
    \label{fig:nuecc:other}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=5cm]{NueCCsel/Images/run1/bdt_1.pdf}
    \includegraphics[height=5cm]{NueCCsel/Images/run1/pre_e_score.pdf}
    \includegraphics[height=5cm]{NueCCsel/Images/run1/bdt_2.pdf}
    \includegraphics[height=5cm]{NueCCsel/Images/run1/pre_daughter_score.pdf}
    \caption{Variables used to do the final selection and event classification.}
    \label{fig:nuecc:event}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{NueCCsel/Images/run1/nue_shower_energy_y.pdf} \hfill
    \includegraphics[width=0.48\textwidth]{NueCCsel/Images/run1/nue_shower_theta.pdf}
    \caption{Distributions of the electron candidate after the final selection.}
    \label{fig:nuecc:after}
\end{figure}
\clearpage
...
\subsection{NuMu CC Inclusive (Constraint Selection) }
\label{sec:numu:constr}
This section contains all the MC-Data comparison plots for Section \ref{ssec:NuMUCCsel:constr}).
