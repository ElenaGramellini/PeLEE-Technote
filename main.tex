...
\begin{itemize}
    \item \textbf{shower track-fitted fraction} (variable: \emph{trfit}, figure~\ref{fig:nue:variables:trkfit}) This quantity is calculated as the fraction of all 3D spacepoints in a shower that are successfully fit with the shower track-fitter algorithm. Tracks, comprised of a single contiguous segment, will be fit in their entire length, leading to a fraction of 1. Showers, with several branches and split charge deposition segments, will have a fraction that is less then one.
...
\begin{center}
    ...
    \caption{\label{fig:nue:variables:secondshowerevd} second shower recovery example }
    \end{subfigure}
    \begin{subfigure}[b]{0.35\textwidth}
    \centering
