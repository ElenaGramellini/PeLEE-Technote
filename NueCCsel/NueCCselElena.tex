...
The preselection builds on top of the SliceID (\cref{sec:sliceID}). At the preselection stage, the following cuts are applied:
\begin{itemize}
...
%\cref{fig:pre_vtx,fig:pre_shower_E_pdg} show the agreement in the vertex location and kinematics of the electron candidate shower at this stage of the selection.  \textcolor{blue}{Why are the vertex plots important? Are the calo plots done without the MC recalibration?}
...
Even if a number of variables contribute to the BDT (see backup \cref{fig:e_cand_dedx} for the d$E$/d$x$ examples),  we highlight here the  d$E$/d$x$ distribution on  collection plane (left side of \cref{fig:e_cand_Calo}) which shows the cleanest e/$\gamma$ separation. The response of the BDT is shown in the right panel of \cref{fig:e_cand_dist}. The complete list of variables used to train the BDT for electron identification is: \emph{shower\_vtx\_dist}, \emph{nObjHits\_Y},  \emph{shr\_tkfit\_2cm\_dedx\_Y},  \emph{shr\_tkfit\_2cm\_dedx\_ALL},  \emph{shr\_tkfit\_gap10\_dedx\_Y}, \emph{shrmoliereavg}, \emph{secondshower\_Y\_vtxdist}, \emph{subcluster}, \emph{ismerged}.
...
Apart from the electron candidate particle, there are other particles in the slice. In this section we study each of those PFParticles separately, except the electron candidate shower described in the previous section. Particle by particle a BDT is used to split the particle into two categories:
\begin{enumerate}
